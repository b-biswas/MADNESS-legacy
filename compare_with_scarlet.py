...
for i in range(len(blend['blend_list'])):
    ...
    convergence_criterion = tfp.optimizer.convergence_criteria.LossNotDecreasing(atol=.00001*45*45*len(blends)*3, min_num_steps=50, window_size=20)
    #convergence_criterion = None
    lr_scheduler = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=0.08, decay_steps=15, decay_rate=0.8, staircase=True)
...
madness_residual_field = deb.compute_residual(blend['blend_images'][0], use_scatter_and_sub=True).numpy()
...
for i in range(len(scarlet_predictions[0])):
    ...
    for band in range(6):
        ...
        flux, fluxerr, flag = sep.sum_circle(actual_galaxy, [blend['blend_list'][0]['x_peak'][i]], [blend['blend_list'][0]['y_peak'][i]], 3.0, err=bkgrms)
        print(flux)
        ...
        flux, fluxerr, flag = sep.sum_circle(scarlet_galaxy, [blend['blend_list'][0]['x_peak'][i]], [blend['blend_list'][0]['y_peak'][i]], 3.0, err=bkgrms)
        print(flux)
        ...
        # madness galaxy
        madness_galaxy = madness_residual_field + np.pad(deb.components[i], padding_infos[i])
        madness_galaxy = madness_galaxy[:, :, band].copy(order='C')
        flux, fluxerr, flag = sep.sum_circle(madness_galaxy, [blend['blend_list'][0]['x_peak'][i]], [blend['blend_list'][0]['y_peak'][i]], 3.0, err=bkgrms)
...
plt.hist(np.abs((madness_gal_fluxes-actual_gal_fluxes)/actual_gal_fluxes), bins=bins, alpha=0.5, label="MADNESS");
plt.hist(np.abs((scarlet_gal_fluxes-actual_gal_fluxes)/actual_gal_fluxes), bins=bins, alpha=0.5, label="scarlet");
